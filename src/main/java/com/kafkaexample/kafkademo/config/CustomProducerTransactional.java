 ...
 * 可能产生的问题：数据重复！
 *
 * 至少一次：= ACKS级别设置为-1+分区副本大于等于2+ISR里应答的最小副本数量大于等于2
 * 最多一次：= ACKS级别设置为0
 * 精确一次：= 幂等性+至少一次，只保证单分区单会话内的精确一次，要想保证多分区多会话内还需开启事务
 * 对于一些重要数据，比如和钱相关的数据，要求数据既不能重复也不丢失，
 * kafka0.11版本以后引入，引入了一项重大特性：幂等性是事务 保证精确一次
 *
 * 必须将acks设置为all/-1才能使用幂等生产者。否则我们不能保证幂等性
 * 开启事务之前必须开启幂等性。
 *
 * 如何使用幂等性：
 * 开启参数properties.put(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG,true);
 * 可不用配置，默认true
 */
public class CustomProducerTransactional {
    public static void main(String[] args) {
        ...
        try {
            //2.发送数据
            for (int i = 0; i < 5; i++) {
                kafkaProducer.send(new ProducerRecord<>("first",1,"","zzzsa"));
            }
            //处理成功提交事务
            kafkaProducer.commitTransaction();
        }catch (Exception e){
            System.out.println("sdsdsdqqq");
            kafkaProducer.abortTransaction();//处理失败回滚事务
        }finally {
            //3.关闭连接
            kafkaProducer.close();
        }
